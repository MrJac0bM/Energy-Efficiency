...
Ejecuta:
  1. src/exploration.py   -> EDA y visualizaciones 
  2. src/transformation.py -> Limpieza y generación de data/processed 
  3. src/regression.py     -> Modelo de regresión lineal 
  4. src/regression_sklearn.py -> Modelo Random Forest 
"""
...
SCRIPTS = {
    ...
    "regression_sklearn": SRC / "regression_sklearn.py",
}
...
def run_script(name: str, verbose: bool = True) -> None:
    ...
    try:
        ...
        if result.returncode != 0:
            print(
                f"[ERROR] {name} falló con código {result.returncode}",
                file=sys.stderr,
                flush=True,
            )
...
def main():
    ...
    if not (ROOT / "data" / "raw" / "energy_efficiency.xlsx").exists():
        print(
            "[WARN] No se encontró data/raw/energy_efficiency.xlsx",
            file=sys.stderr,
            flush=True,
        )
